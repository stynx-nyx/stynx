...
describe('worklist validation', () => {
  it('accepts canonical queue permissions and an extensible strategy key', () => {
    expect(createQueueSchema.parse({
      code: 'rait-review',
      name: 'RAIT review',
  ...
  it('requires exactly one queue default deadline shape', () => {
    ...
    }).defaultDeadline).toEqual({ kind: 'elapsed', seconds: 3_600 });
  });

  it('validates polymorphic refs and absolute or business-day item deadlines', () => {
    expect(enqueueWorkItemSchema.parse({
    ...
    }).deadline).toMatchObject({ kind: 'absolute' });
    ...
    }).deadline).toEqual({ kind: 'business_days', businessDays: 10 });
  ...
  it('constrains worker state and supervisor override inputs', () => {
    expect(workerStateSchema.parse({
    ...
    expect(() => supervisorOverrideSchema.parse({
