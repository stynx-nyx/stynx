...
Kept as historical archive (per `git blame` and `git log`, these tasks were the v1.0 implementation drivers and provide context for why certain stynx packages exist in their current shape). They do NOT become DEVAI invariants — they're closed work items, not active rules. If a regression surfaces against the discipline any of these established, file an invariant under `docs/architecture/invariants/` referencing the relevant GAP-\* file by path.
