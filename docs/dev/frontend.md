...
- Frontend Vitest tests live alongside each Angular package under `packages-web/angular*/test`. The previous `test/frontend` tree (vestigial — referenced a `frontend/` directory that never existed at the repo root) was removed during the Vitest cutover.
