...
- Every bootstrap or sync run compares `backend/.env` with `../porm/backend/.env`. Missing keys are surfaced with masked values so you can copy them across quickly.
...
1. Execute `npx tsx bootstrap/index.ts configure --sync-env` to reconcile configuration against porm.
