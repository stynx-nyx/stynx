...
- Building or modernizing an Angular app: start here, then read the referenced `@stynx-nyx/*` package docs.
- Porting an existing non-Angular frontend: keep the app framework and use [`@stynx-nyx/sdk`](/docs/packages-web/sdk); the porting-specific checklist lives in [`FRONTEND-PATTERNS`](/docs/adopters/stynx/porting-pack/FRONTEND-PATTERNS).
...
- Foundation providers from [`@stynx-nyx/angular`](/docs/packages-web/angular).
- Auth, OIDC, route guards, permission directives, and session exchange from [`@stynx-nyx/angular-auth`](/docs/packages-web/angular-auth).
- Tenant context and tenant header plumbing from [`@stynx-nyx/angular-tenancy`](/docs/packages-web/angular-tenancy).
- Flow routes from [`@stynx-nyx/angular-flow`](/docs/packages-web/angular-flow).
- IAM routes from [`@stynx-nyx/angular-iam`](/docs/packages-web/angular-iam).
...
`@stynx-nyx/angular` registers the request-id, auth, and error interceptors. `@stynx-nyx/angular-tenancy` registers tenant context and the tenant interceptor through the tenancy provider used by the foundation module. Do not hand-write those headers in feature pages unless you are deliberately outside Angular and using the SDK-only path.

For non-Angular adopters, [`@stynx-nyx/sdk`](/docs/packages-web/sdk) is the contract. Provide an `AuthProvider`, `TenantProvider`, `baseUrl`, and `fetchFn`; keep React/Vue/Svelte/Node state management in the host app.

## Auth

Production auth uses the real OIDC adapter/backend from [`@stynx-nyx/angular-auth`](/docs/packages-web/angular-auth) or an audited equivalent that implements the same package contracts.
...
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Typed HTTP without Angular                         | [`@stynx-nyx/sdk`](/docs/packages-web/sdk)                                                                                               | Porting-pack SDK path in [`FRONTEND-PATTERNS`](/docs/adopters/stynx/porting-pack/FRONTEND-PATTERNS) |
| Angular foundation and interceptors                | [`@stynx-nyx/angular`](/docs/packages-web/angular)                                                                                       | `reference/web/src/app/reference-web.module.ts`                                                     |
| Login, session, route guards, permission directive | [`@stynx-nyx/angular-auth`](/docs/packages-web/angular-auth)                                                                             | `reference/web/src/app/pages/login.page.ts`, `reference/web/src/app/app.routes.ts`                  |
| Tenant selector and tenant header context          | [`@stynx-nyx/angular-tenancy`](/docs/packages-web/angular-tenancy)                                                                       | `reference/web/src/app/pages/tenant-selection.page.ts`                                              |
| Flow design/runtime/fill/tasks                     | [`@stynx-nyx/angular-flow`](/docs/packages-web/angular-flow)                                                                             | `/flow/*` route mount in `reference/web/src/app/app.routes.ts`                                      |
| IAM users, roles, groups, permissions              | [`@stynx-nyx/angular-iam`](/docs/packages-web/angular-iam)                                                                               | `/admin/*` route mount in `reference/web/src/app/app.routes.ts`                                     |
| I18n catalog, translate pipe, locale switcher      | [`@stynx-nyx/angular-i18n`](/docs/packages-web/angular-i18n)                                                                             | `reference/web/src/app/core/reference-web-i18n.service.ts`                                          |
| Presigned document upload/download                 | [`@stynx-nyx/angular-storage`](/docs/packages-web/angular-storage)                                                                       | `reference/web/src/app/pages/records.page.ts`                                                       |
| Trash/restore UI                                   | [`@stynx-nyx/angular-trash`](/docs/packages-web/angular-trash)                                                                           | `reference/web/src/app/pages/trash.page.ts`                                                         |
| Profile and sessions surfaces                      | [`@stynx-nyx/angular-profile`](/docs/packages-web/angular-profile), [`@stynx-nyx/angular-sessions`](/docs/packages-web/angular-sessions) | `reference/web/src/app/pages/dashboard.page.ts`                                                     |
| Shared banners, tables, dialogs, toasts            | [`@stynx-nyx/angular-ui`](/docs/packages-web/angular-ui)                                                                                 | Reference pages under `reference/web/src/app/pages/`                                                |
...
Storage uses the package upload flow: initiate, upload to the presigned URL, then complete. See [`@stynx-nyx/angular-storage`](/docs/packages-web/angular-storage) for the component/service API and `reference/web/src/app/pages/records.page.ts` for page composition.
