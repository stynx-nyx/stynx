...
- [x] `InMemoryRateLimitMetrics` has `recordLatency()`, `latencyPercentile()`, `histogramSnapshot()`
- [x] `RateLimitMetricsSink` interface has optional `recordLatency?`
- [x] Rate-limit guard emits `recordLatency` on each check
- [x] Auth permission cache accepts `driftResyncIntervalMs` and proactively refreshes stale entries
- [x] Background interval uses `.unref()` so it does not prevent clean process exit
- [x] Unit tests cover p50/p95/p99 percentile calculation and drift re-sync trigger
