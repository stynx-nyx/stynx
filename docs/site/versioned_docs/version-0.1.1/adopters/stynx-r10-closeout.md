...
- Package: `@stynx-nyx/signature` v0.1.0 workspace state with
...
- Migration: SGP can replace local digest helpers in
  ...
  after its own R10 adoption pass updates imports from `@stynx-nyx/signature`.
- Fixture compat: all new helpers are deterministic and provider-free. The
  ...
  goldens.
- Commands:
  - `pnpm --filter @stynx-nyx/signature test`
  - `pnpm --filter @stynx-nyx/signature typecheck`
...
- Owner decisions: PDF/A conformance remains adopter-owned in R10. See
  [`ADR-PDF-A-BOUNDARY`](/docs/adr/ADR-PDF-A-BOUNDARY).
