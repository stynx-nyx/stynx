...
```ts
import {} from /* public exports */ '@stynx-nyx/i18n';
```
...
```ts
import { ErrorTranslatorService } from '@stynx-nyx/i18n';
...
```sh
pnpm --filter @stynx-nyx/i18n build
pnpm --filter @stynx-nyx/i18n test
STYNX_TEST_PG_HOST=localhost pnpm --filter @stynx-nyx/i18n test:int
```
