...
DEVAI-owned record commands write artifacts under `internal DEVAI state artifact (not published)`
unless an explicit output path is supplied:

```
internal DEVAI state artifact
```
...
| -------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------- |
| `pnpm test:matrix*` / `devai render-matrix`        | `internal DEVAI state artifact (not published)`                   | Canonical DEVAI matrix rendering.        |
| `pnpm test:evidence` / `devai evidence-emit`       | `internal DEVAI state artifact (not published)`                   | Appends evidence-chain records.          |
| `pnpm test:coverage`                               | per-target `coverage summary JSON (not published)` under coverage | Uses `devai coverage-aggregate`.         |
...
parse stdout. They should instead read `generated test evidence summary (not published)`:

```js
// example sensor adapter
import { readFileSync } from 'node:fs';
const evidence = JSON.parse(readFileSync('generated test evidence summary', 'utf8'));
