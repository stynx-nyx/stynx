...
**Related:** Upstream amendment record at [`external DEVAI sibling checkout reference (not published)`](#external-devai-reference) (Article 40 first invocation, R14 close);
upstream IA law [`external DEVAI sibling checkout reference (not published)`](#external-devai-reference);
canonical migration guide `https://aarusso-nyx.github.io/devai/docs/adopters/migrating-to-0.2.0`;
triggering proposal [`internal round-tracking note (not published)`](#internal-round-tracking-note);
round plan [`internal round-tracking note (not published)`](#internal-round-tracking-note);
...
The migration is **not** time-critical — DEVAI continues to serve 0.1.1 indefinitely — but it is the precondition for STYNX receiving any future DEVAI substrate that assumes 0.2.0 path semantics. The triggering proposal (`internal round-tracking note (not published)`), authored on the DEVAI side immediately post-R14-close, recommends an eight-wave round; this ADR ratifies the migration and locks the four non-mechanical decisions that round needs to execute against.
...
| `internal work note (not published)` | (no move; not under 0.2.0 enumeration)               | `internal work note (not published)` is operational artifacts (the `2026-05-24-*.md` notes). Stays put as out-of-IA operational scratch — confirmed against 0.2.0 enumeration which does not name `work/`. |
...
- **Round-artifact path divergence persists.** STYNX stores round artifacts at `internal round-tracking note (not published)`; DEVAI stores them at `internal work note (not published)`. The `.gitignore` exception added in W01 is `!internal round-tracking note`, not the canon's `!internal work note.log`. Harmonisation is out of scope; flag for a future round if desired.
...
- **(e) Move `internal work note (not published)` into the IA.** Rejected per Decision B. `internal work note (not published)` is operational scratch (per-day `.md` notes from R14), not IA content. Leaving it out-of-IA matches DEVAI's own treatment.
...
- **STYNX R15 Plan** — `internal round-tracking note (not published)`.
- **STYNX R15 orchestrator** — `internal round-tracking note (not published)`.
- **STYNX R15 workers** — `internal round-tracking note (not published)`.
- **Triggering proposal** — `internal round-tracking note (not published)`.
- **DEVAI R14 round artifacts** — `external DEVAI sibling checkout reference (not published)`../../../../devai/scripts/r14-\*.mjs`+`external DEVAI sibling checkout reference (not published)`.
