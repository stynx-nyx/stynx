**@stynx/integration-adapter**

---

# @stynx/integration-adapter

Public integration adapter framework exports.

## Classes

- [InMemoryCircuitBreaker](classes/InMemoryCircuitBreaker.md)
- [InMemoryIdempotencyStore](classes/InMemoryIdempotencyStore.md)
- [IntegrationAdapter](classes/IntegrationAdapter.md)

## Interfaces

- [CircuitBreaker](interfaces/CircuitBreaker.md)
- [CircuitBreakerPolicy](interfaces/CircuitBreakerPolicy.md)
- [CircuitSnapshot](interfaces/CircuitSnapshot.md)
- [IdempotencyStore](interfaces/IdempotencyStore.md)
- [IntegrationAdapterOptions](interfaces/IntegrationAdapterOptions.md)
- [IntegrationContext](interfaces/IntegrationContext.md)
- [IntegrationTelemetry](interfaces/IntegrationTelemetry.md)
- [IntegrationTelemetryEvent](interfaces/IntegrationTelemetryEvent.md)
- [RetryPolicy](interfaces/RetryPolicy.md)

## Type Aliases

- [CircuitState](type-aliases/CircuitState.md)
