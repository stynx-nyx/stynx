**@stynx/feature-flags**

---

# @stynx/feature-flags

Public feature-flag provider exports.

## Classes

- [FeatureFlagsService](classes/FeatureFlagsService.md)
- [InMemoryFeatureFlagProvider](classes/InMemoryFeatureFlagProvider.md)
- [JsonFileFeatureFlagProvider](classes/JsonFileFeatureFlagProvider.md)

## Interfaces

- [FeatureFlagProvider](interfaces/FeatureFlagProvider.md)
- [FlagContext](interfaces/FlagContext.md)
- [FlagDefinition](interfaces/FlagDefinition.md)
- [FlagEvaluation](interfaces/FlagEvaluation.md)
- [FlagSet](interfaces/FlagSet.md)

## Type Aliases

- [FlagPrimitive](type-aliases/FlagPrimitive.md)
- [FlagValue](type-aliases/FlagValue.md)
