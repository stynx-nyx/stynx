...
**Package:** `@stynx/pdf-a@0.1.0`, `@stynx/pdf-a-vera-docker@0.1.0`
**Author roles:** Architect, Engineer, Inspector, per DEVAI Article 6.

## Surface Changes

- Add `@stynx/pdf-a` with `PdfAValidator`, validation result types, telemetry
  metric names, `NoopPdfAValidator`, and `StrictPdfAValidator`.
- Add `@stynx/pdf-a-vera-docker` with the digest-pinned veraPDF Docker adapter,
  JSON report normalization, Docker timeout handling, fixture corpus, and bench
  gate.
- No existing `@stynx/pdf` public surface is modified.

## Migration Notes

Existing `@stynx/pdf` consumers do not need to migrate. PDF/A validation is
...
```sh
pnpm --filter @stynx/pdf-a build test lint typecheck
pnpm --filter @stynx/pdf-a-vera-docker build test lint typecheck bench
```
