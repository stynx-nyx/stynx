...
Ran `devai inv-adherence-reverse --repo-root . --inventory internal DEVAI state artifact --human`. Result: **0 surfaces; 0 claimed, 0 orphan**.
...
- (b) `inv-adherence-reverse` should consume the per-slice sensor body files directly (e.g., `internal DEVAI state artifact (not published)`) rather than the aggregate.
...
- `internal DEVAI state artifact (not published)` — first aggregate inventory.json commit (was missing). Generated via `devai inv-regen --no-git`.
