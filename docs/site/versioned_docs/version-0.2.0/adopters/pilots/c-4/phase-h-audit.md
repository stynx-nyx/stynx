...
| `f1-paths-present`     | ✗ FAIL  | Missing `docs/framework/schemas/` (DEVAI's own schemas directory; adopters use `external DEVAI sibling checkout reference (not published)`). |
| `schemas-loadable`     | ✗ FAIL  | `docs/framework/schemas/` directory missing (cascade from above).                                                                            |
...
`CLAUDE.md` was authored at the stynx repo root (Phase H deliverable) — a session-governance pointer mirroring DEVAI's CLAUDE.md but pointing at the sibling `external DEVAI sibling checkout reference (not published)` checkout for the canonical reading order. The doctor's `agents-claude-sync` check also requires the file to mention five specific tokens (Constitution Article 6, the five role names, the canonical reading-order sources `README.md` / `CONSTITUTION.md` / `BUILD-PLAN.md` / `DESIGN-DECISIONS.md` / `docs/schemas`); the new file includes all of them by reference.
...
| `blueprint-validate docs/framework/product/draft/blueprints/demo-bookmark.json`              | ✓ PASS        | Phase C blueprint validated cleanly post-rewrites.                                                                  |
