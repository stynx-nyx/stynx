...
Seven commits land into stynx. Zero changes under `external DEVAI sibling checkout reference (not published)`.

---

## 1. Sensor counts (from `internal DEVAI state artifact (not published)`)
...
## 2. Candidate invariants (from `internal DEVAI state artifact (not published)`, A.4)
...
**Recommendation for the pack** (Architect call against `external DEVAI sibling checkout reference (not published)`, deferred to a separate DEVAI session per session boundary discipline):

- Add `apps/reference-api`, `apps/reference-web`, and (post-A.0b) `reference/api`, `reference/web` as alternate signal directories.
- Document the alternate-layout pattern in `external DEVAI sibling checkout reference (not published)`.
...
**Where:** `external DEVAI sibling checkout reference (not published)` (mock client) + `external DEVAI sibling checkout reference (not published)` (writer-skill response parser).
...
**Evidence:** `internal DEVAI state artifact (not published)`, `internal DEVAI state artifact (not published)`, `internal DEVAI state artifact (not published)`.
...
**Where:** `external DEVAI sibling checkout reference (not published)` (or wherever the routes walker lives).
...
**Where:** `external DEVAI sibling checkout reference (not published)` only knows three families (`mock`, `claude`, `codex`) and both real backends require an API key (`ANTHROPIC_API_KEY` or `OPENAI_API_KEY`).
...
**Reference implementation:** `/tmp/devai-prompt-&#123;compose,merge&#125;.mjs` from this session, plus the three commits worth of telemetry now in `internal DEVAI state artifact (not published)`.

### D-A-5 — lint-staged runs prettier on `internal DEVAI state artifact (not published)`

**Where:** stynx's lint-staged config formats every staged JSON. The DEVAI evidence chain (`internal DEVAI state artifact (not published)`) is hash-chained per Article 32; if a future commit triggers prettier rewriting an existing chained record, the chain breaks.
**Mitigation today:** Phase A only adds new state files; nothing existing is rewritten, so the chain is intact.
**Long-term:** add `internal DEVAI state artifact (not published)` to lint-staged ignore (or `prettier --ignore-path` augment). Phase G consolidation work.
...
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GOVERNANCE.md` (stub pointing at `docs/meta/gov/`)                                                  | DEVAI Constitution + `docs/framework/arch/`, `docs/meta/security/`, `docs/meta/ops/` substrates       | Replace with a one-line "see `.devai/CONSTITUTION.md` and `docs/framework/arch/`" pointer.                                                                                                           |
...
| `docs/meta/gov/&#123;health, audit, compliance&#125;`                                                | DEVAI scorecard + evidence chain + auditor outputs in `internal DEVAI state artifact (not published)` | Migrate canonical compliance/health/audit content into DEVAI's substrates; leave stynx tree as transitional links.                                                                                   |
| `internal work note (not published)`                                                                 | DEVAI's `internal DEVAI state artifact (not published)` (sensor bodies, candidates, agent-runs)       | Largely redundant once DEVAI is wired; treat as scratch space and gitignore.                                                                                                                         |
| `specs/` (15 normative specs incl. ADRs)                                                             | DEVAI `docs/framework/arch/` + `docs/meta/adr/`                                                       | One-time migration: move spec content into `docs/framework/arch/invariants/` and ADRs into `docs/meta/adr/`. Keep `/specs/` as a transitional symlink or remove after migration.                     |
...
Phase B is "promote candidate invariants from `internal DEVAI state artifact (not published)` into `docs/framework/arch/invariants/INV-&lt;DOMAIN&gt;-NNN.json`." Recommended priority order, given Phase A signal:
...
4. **A new invariant family stynx should consider authoring:** "every published endpoint has a documented use-case in `docs/framework/product/`." This is a Phase B _Owner-authored_ invariant that closes the gap inventory currently surfaces, rather than a sensor-derived candidate. Out of scope to draft here.
...
| No edits to `external DEVAI sibling checkout reference (not published)` | ✅ confirmed clean — `git -C ../devai status` (recommend the user verify)                     |
| No invariant promotion (Phase B's job)                                  | ✅ candidates remain under `internal DEVAI state artifact (not published)`                    |
