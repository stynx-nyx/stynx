...
**Where:** [`external DEVAI sibling checkout reference (not published)`](#external-devai-reference) lines 310-311.

```ts
const routesPath = opts.routesPath ?? join(opts.repoRoot, 'internal DEVAI state artifact');
```

**Symptom:** Stynx is an Angular shop; `sense-routes` writes its inventory to `internal DEVAI state artifact (not published)` (15 routes captured). `sense-coverage` then can't find `routes-react.json` and emits:

```
inventory:coverage [inventory_coverage]: REVIEW
  [warning] COVERAGE_REQUIRES_ROUTES: routes-inventory body not found at
  internal DEVAI state artifact Run 'devai sense routes' first.
```
...
**Workaround that works today:** `devai sense-coverage --routes-path internal DEVAI state artifact` PASSes and populates the matrix with all 15 Angular routes. This is how the last committed [`coverage-matrix.json`](../../../../internal DEVAI state artifact) (generatedAt `2026-05-18T00:06:29.650Z`) was produced — with the explicit `--routes-path` override. The bug is purely the default value when no override is provided; a no-arg `devai sense-coverage` against an Angular adopter emits REVIEW with the routes-react.json warning.

**Caller-side mitigation (already in use):** Whatever script generated the 2026-05-18 matrix (likely an S-series automation or skill) must already pass `--routes-path` explicitly. Capturing that invocation in a stynx-controlled wrapper (`pnpm sense:coverage` script, or [`tools/repo-config/devai-shims/`](../../../../tools/repo-config/)) would make the workaround durable across `internal DEVAI state artifact (not published)` regenerations.
...
- **(a) Default to a framework-agnostic glob.** `glob('internal DEVAI state artifact')` and pick the first that exists, or merge all (stynx may eventually carry both an Angular shell + a React reference web).
...
```
"sense:coverage": "devai sense-coverage --repo-root . --routes-path internal DEVAI state artifact --human"
```

`pnpm sense:coverage` PASSed against the current state. Durable across `internal DEVAI state artifact (not published)` regenerations; no DEVAI patch dependency; no symlinks. Future S-series automations and skills were expected to call this script instead of bare `devai sense-coverage`.
...
**Where:** [`external DEVAI sibling checkout reference (not published)`](#external-devai-reference) — the inline action-coverage gate.
...
- Schema source: [`external DEVAI sibling checkout reference (not published)`](#external-devai-reference) (id regex, domain pattern).
