...
4. Execute the original `porm` Flow consumer cutover to `@stynx/flow` and `@stynx-web/angular-flow` using [porm-flow-deprecation-readiness.md](./porm-flow-deprecation-readiness) once the consuming repo is ready for the dependency swap.
