...
The audit at `internal work note (not published)` (commit `457da90`, 2026-04-27 morning)
...
| ----------------------------------------------- | ------------ | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FIND-001 (`@stynx-nyx/contracts` missing)       | BLOCKER      | **CLOSED**       | `packages/contracts/package.json` (`@stynx-nyx/contracts@0.2.0`, 8 exports)                                                                             |
| FIND-002 (`@stynx-nyx/angular-tenancy` missing) | BLOCKER      | **CLOSED**       | `packages-web/angular-tenancy/package.json` (6 exports)                                                                                                 |
...
| FIND-006 (`@stech/*` legacy packages)           | MAJOR        | **CLOSED**       | `pnpm -r ls` shows no `@stech/*`; `@stynx-nyx/backend@0.2.0` is the rationalized successor                                                              |
...
- FIND-010 (`@stynx-nyx/privacy` I3 violation — direct `@aws-sdk/client-s3`).
...
| `docs/`                                                      | Docusaurus + `internal work note (not published)` (audit, rationalization, specs, prompts) |
...
| `STYNX-API-DATA.md`              | 618    | `@stynx-nyx/data` API contract         |
...
| ------------------------ | ------- | ------- | ----- | ------ |
| `@stynx-nyx/audit`       | 0.1.0   | 8       | 3     | Y      |
| `@stynx-nyx/auth`        | 0.1.0   | 20      | 12    | Y      |
| `@stynx-nyx/backend`     | 0.2.0   | 44      | 0     | Y      |
| `@stynx-nyx/cli`         | 0.1.0   | 7       | 3     | Y      |
| `@stynx-nyx/contracts`   | 0.2.0   | 8       | 0     | Y      |
| `@stynx-nyx/core`        | 0.1.0   | 11      | 5     | Y      |
| `@stynx-nyx/data`        | 0.1.0   | 12      | 6     | Y      |
| `@stynx-nyx/health`      | 0.1.0   | 6       | 4     | Y      |
| `@stynx-nyx/i18n`        | 0.1.0   | 9       | 1     | Y      |
| `@stynx-nyx/idempotency` | 0.1.0   | 10      | 2     | Y      |
| `@stynx-nyx/logging`     | 0.1.0   | 6       | 5     | Y      |
| `@stynx-nyx/privacy`     | 0.1.0   | 9       | 2     | Y      |
| `@stynx-nyx/ratelimit`   | 0.1.0   | 10      | 3     | Y      |
| `@stynx-nyx/sessions`    | 0.1.0   | 10      | 2     | Y      |
| `@stynx-nyx/storage`     | 0.1.0   | 7       | 3     | Y      |
| `@stynx-nyx/tenancy`     | 0.1.0   | 10      | 4     | Y      |
| `@stynx-nyx/testing`     | 0.1.0   | 8       | 1     | Y      |

**Note:** `@stynx-nyx/backend` and `@stynx-nyx/contracts` are at 0.2.0; the rest
at 0.1.0 — likely the rationalization seam from `@stech/*` predecessors.

**Spec drift:** spec §3 lists 16 backend packages (no `@stynx-nyx/backend`).
The repo adds `@stynx-nyx/backend` as an aggregator/composition module —
...
Treat `@stynx-nyx/backend` as STABLE-but-undocumented-in-spec; subsequent
...
| Name                          | Version | Exports | Tests | README |
| ----------------------------- | ------- | ------- | ----- | ------ |
| `@stynx-nyx/angular`          | 0.1.0   | 12      | 1     | Y      |
| `@stynx-nyx/angular-auth`     | 0.1.0   | 13      | 1     | Y      |
| `@stynx-nyx/angular-i18n`     | 0.1.0   | 6       | 1     | Y      |
| `@stynx-nyx/angular-profile`  | 0.1.0   | 3       | 1     | Y      |
| `@stynx-nyx/angular-sessions` | 0.1.0   | 2       | 1     | Y      |
| `@stynx-nyx/angular-storage`  | 0.1.0   | 5       | 1     | Y      |
| `@stynx-nyx/angular-tenancy`  | 0.1.0   | 6       | 1     | Y      |
| `@stynx-nyx/angular-trash`    | 0.1.0   | 2       | 1     | Y      |
| `@stynx-nyx/angular-ui`       | 0.1.0   | 8       | 1     | Y      |
| `@stynx-nyx/sdk`              | 0.1.0   | 14      | 1     | Y      |

All ten spec'd frontend packages present. `@stynx-nyx/angular-tenancy`
...
Direct invocation via `pnpm --filter @stynx-nyx/cli exec stynx --help`
...
## 10. `@stynx-nyx/data` exported surface (verified at HEAD)
...
`Database as CoreDatabase` from `@stynx-nyx/core` and adds tenancy / actor
...
1. **`@stynx-nyx/backend` is undocumented in spec §3** but is the actual
...
6. **`@stynx-nyx/privacy` I3 status (FIND-010) re-verification owed.**
...
8. **Operations runbooks (FIND-031)** — discovery did not re-check
   `docs/meta/ops/`. Severity: MAJOR.

## 12. Audit findings carrying into porting concerns

From `internal work note (not published)`, the items consuming
...
- **FIND-011** — doctor empty output (if live, doctor cannot be the
  port's "done" gate).
- **FIND-015** — LGPD pipeline thinly tested (`@stynx-nyx/privacy` has 2
...
- **FIND-031** — operations runbooks; re-check `docs/meta/ops/`.
