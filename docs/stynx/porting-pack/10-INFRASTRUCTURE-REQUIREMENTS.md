...
| `STYNX_REDIS_URL`                                                                                                    | yes (prod)                               | `redis://127.0.0.1:6379` (sessions only)                 | `app.module.ts:188-194,199,236-241,246-252` and `process.env.STYNX_REDIS_URL`. Used by `@stynx/auth`, `@stynx/sessions`, rate-limit, idempotency.                                                                               |
...
| `DOCS_BUCKET` / `KMS_DOCS_KEY_ID` env names not aligned with `@stynx/storage` consumer (`STYNX_STORAGE_BUCKET` / `STYNX_KMS_ALIAS`) | blocker               | `compute-stack.ts:91-92` vs `app.module.ts:217-220`      |
