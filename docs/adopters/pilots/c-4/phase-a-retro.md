...
| `docs/meta/gov/{health, audit, compliance}`                                                          | DEVAI scorecard + evidence chain + auditor outputs in `.devai/state/`                           | Migrate canonical compliance/health/audit content into DEVAI's substrates; leave stynx tree as transitional links.                                                                                   |
...
Phase B is "promote candidate invariants from `.devai/state/inv-candidates/` into `docs/framework/arch/invariants/INV-<DOMAIN>-NNN.json`." Recommended priority order, given Phase A signal:
