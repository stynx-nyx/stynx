...
| `check-adrs`                           | ✅ OK (0 file(s), 0 error(s))           | Stynx's 2 ADRs at `docs/adr/` are pre-DEVAI format; `check-adrs` finds nothing schema-conformant to validate. Filed as F-16 for a future Architect session. |
...
The use-cases at `docs/product/use-cases/stynx-reference-app{,-extended}.json` validate against the schema and reference correct endpoint + route IDs. But `sense-coverage` doesn't read them to populate `coverage-matrix.json#/links[]`. Either:

- (a) `sense-coverage` should look for `docs/product/use-cases/*.json` files and synthesize links, OR
...
| **D-A-18** | S10 (this audit) | sense-coverage reads `docs/product/use-cases/*.json` to populate links |
...
| **F-16**         | This audit    | Re-author ADRs at `docs/adr/` in DEVAI's schema-conformant form |
