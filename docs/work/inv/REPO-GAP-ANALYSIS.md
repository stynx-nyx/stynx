...
| `test/backend/vitest.config.ts`                                               | divergent | backend tests target the legacy app surface rather than the target package/reference-app topology                            |
...
| `test/db/vitest.config.ts`                                                    | partial   | database tests contain useful assertions and migration evidence, but they currently validate a legacy schema contract        |
...
| `test/frontend/Vitest.config.cjs`                                             | divergent | frontend tests target the legacy app surface rather than the target `packages-web/*` plus `reference-web` topology           |
| `test/frontend/vitest.config.ts`                                              | divergent | frontend tests target the legacy app surface rather than the target `packages-web/*` plus `reference-web` topology           |
| `test/frontend/setup-Vitest.ts`                                               | divergent | frontend tests target the legacy app surface rather than the target `packages-web/*` plus `reference-web` topology           |
| `test/frontend/tsconfig.Vitest.json`                                          | divergent | frontend tests target the legacy app surface rather than the target `packages-web/*` plus `reference-web` topology           |
...
| `test/packages/vitest.config.ts`                                              | divergent | file does not align cleanly with the v1 topology and appears to belong to the transitional or legacy implementation surface  |
