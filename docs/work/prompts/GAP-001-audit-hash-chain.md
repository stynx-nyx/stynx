...
- [ ] `audit.events` has `previous_hash` (nullable) and `row_hash` (generated stored) columns
- [ ] `audit.write()` always populates `previous_hash` from the previous row's `row_hash`
- [ ] `audit.fn_log_dml()` trigger uses the same pattern
- [ ] `auditService.verifyChain()` returns `{ valid: false }` when any row is mutated
- [ ] Integration test covers chain creation and tamper detection
- [ ] `pnpm build`, `pnpm test:unit`, `pnpm test:int`, `pnpm lint` all green
