...
- [ ] `ConfigKeyMetadata`, `ConfigOwner`, `AnnotatedSchema` are exported from `@stynx/core`
- [ ] `ConfigOwnershipViolationError` is exported from `@stynx/core`
- [ ] `StynxCoreModuleOptions` accepts `configMetadata`
- [ ] `loadStynxConfiguration` throws `ConfigOwnershipViolationError` when a required key is absent
- [ ] Unit test covers violation and non-violation paths
