...
- [ ] `SessionExchangeOptions` and `SessionExchangeResult` exported from `@stynx/sessions`
- [ ] `SessionExchangeError` exported from `@stynx/sessions` with typed `code` field
- [ ] `SessionService.exchange()` revokes the source session before issuing the replacement
- [ ] Owner mismatch and inactive session are each rejected with typed errors
- [ ] Unit test covers happy path and all three error cases
