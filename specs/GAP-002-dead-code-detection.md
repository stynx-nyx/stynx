...
- [x] `knip` and `depcheck` are in root `devDependencies`
- [x] `pnpm lint:deadcode` runs without error on a clean repo
- [x] `pnpm lint:deps` reports zero orphaned deps
- [x] `turbo.json` has a `lint:deadcode` task entry
- [x] CI `lint` job blocks on both new steps
- [x] `pnpm lint` (turbo) still passes
