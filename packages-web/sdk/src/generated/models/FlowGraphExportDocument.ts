/* generated using openapi-typescript-codegen -- do not edit */
/* istanbul ignore file */
/* tslint:disable */
/* eslint-disable */
import type { FlowJsonObject } from './FlowJsonObject';
export type FlowGraphExportDocument = {
    agentRules: Array<FlowJsonObject>;
    edges: Array<FlowJsonObject>;
    graph: FlowJsonObject;
    nodeFormRules: Array<FlowJsonObject>;
    nodes: Array<FlowJsonObject>;
    policyRules: Array<FlowJsonObject>;
    policySets: Array<FlowJsonObject>;
    transitionEffects: Array<FlowJsonObject>;
};

