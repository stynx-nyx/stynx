...
6. **[`../devai/docs/framework/schemas/`](../devai/docs/framework/schemas/)** — the canonical schemas every JSON instance in stynx (under `.devai/state/`, `docs/framework/arch/invariants/`, `docs/framework/product/draft/blueprints/`) MUST validate against.
...
9. **[`docs/adopters/pilots/c-4/`](docs/adopters/pilots/c-4/)** — pilot retros documenting the C-4 adoption experience (`phase-a-retro.md`, `phase-h-audit.md`, `phase-i-retro.md`, `phase-s7-loop-activation.md`, `phase-s8-tuning-audit.md`, `phase-s10-audit.md`).
...
| **Architect** | `docs/framework/arch/`, `docs/framework/contracts/`, `docs/meta/adr/`, `docs/meta/ops/`, `docs/meta/security/`, joint `docs/framework/glossary/`, `README.md`, `GOVERNANCE.md`, this file |
...
- **Schema instances validate against schemas:** anything matching a schema in `../devai/docs/framework/schemas/` MUST validate against that schema. Schema drift is a hard-gate failure (the Phase E CI workflow `.github/workflows/devai-gates.yml` enforces this for invariants and module-blueprints).
...
- Some `devai doctor` checks (workspace-layout, f1-paths-present, schemas-loadable, constitution-symlink) hardcode DEVAI's monorepo shape (`packages/{cli,core,schemas,sensors,utils}`, root-level `tsconfig.base.json`, `docs/framework/schemas/`, `.devai/constitution.md`). These will fail on adopters today; the underlying gap is filed in [`docs/adopters/pilots/c-4/phase-a-retro.md`](docs/adopters/pilots/c-4/phase-a-retro.md) (D-A-9 from Phase H).
...
- Stynx **inherits** DEVAI's universal invariants (e.g. `INV-INVENTORY-001`, `INV-INVENTORY-002`, `INV-INVENTORY-003`) from `../devai/docs/framework/arch/invariants/`. Stynx's own invariants live under [`docs/framework/arch/invariants/`](docs/framework/arch/invariants/) and SHOULD specialize the universal ones rather than redefine them.
- The repo carries deliberate `TODO_PERMISSION` and `TODO(stynx-adopt)` sentinels injected by `@stynx-nyx/cli` adopt. A repo-wide grep for `TODO` will surface those; they are not unresolved tech debt unless a specific instance is proven outside the generated-adoption pattern.
