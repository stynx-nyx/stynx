...
Mutation testing exists in stynx and is operating: 31 packages carry a `stryker.conf.mjs`, every one of them had a current `.test-results/mutation.json` at the audit date (most recent runs 2026-05-19 23:11 → 2026-05-20 01:39), and the central factory at `tools/stryker/base.mjs` enforces a sane baseline (`coverageAnalysis: 'perTest'`, TypeScript checker, vitest runner, threshold ratchet to `tools/repo-config/test-policy.json`).
...
2. **Two packages sit ≤ 1 point above the break threshold and have concentrated, killable survivors.** `@stynx-nyx/sessions` 60.13 % / break 60 (63 survivors in 3 files, 22 from `ConditionalExpression`, 15 from `StringLiteral`) and `@stynx-nyx/flow` 60.55 % (340 survivors in 3 service files, 168 from `StringLiteral`). One refactor breaks each gate; both have killable hot zones.
...
| Threshold policy                               | yes      | `tools/repo-config/test-policy.json#policies.mutation` + `perPackage` overrides                                                      |
...
| Canonical per-(pkg, level) JSON                | 31 / 31  | every `<pkg>/.test-results/mutation.json` was current at the audit date                                                              |
...
| `thresholds`               | `{ high: T, low: max(60, T-10), break: T }` (`tools/stryker/base.mjs:51-55`)                        | `break == high`, meaning the workspace fails on any score below `high`. This is strict, but the `T` value is the bottleneck: defaults to 60 (`tools/repo-config/test-policy.json#defaults.mutation`). Per-package overrides: `auth/data = strictest (85)`, `tenancy = strict (80)`, `angular-iam/angular-audit = 70`. Every other package uses 60. _Opinion: 60 is permissive for security-tier and state-engine code; sessions, flow, idempotency, and core all qualify and currently sit between 60 % and 73 %._                                                                                                                                                                                                                |
| `timeoutMS`                | unset by default; `@stynx-nyx/data` overrides to `1000` ms (`packages/data/stryker.conf.mjs:9`)     | The 1000 ms override on `@stynx-nyx/data` is **suspicious**. The integration-aware tests in `data` cover testcontainer-backed flows; 1 second is below realistic per-test timing. Evidence: `data`'s mutation.json shows 94 Timeout outcomes (`{Killed:472, CompileError:185, Timeout:94, Survived:20}`). 94 timeouts on 791 valid mutants is **11.9 %** — those are killable mutants being counted as "killed" only by accident (Stryker counts Timeout as killed for scoring, masking gaps). _Opinion: this is the largest single configuration mis-step in the workspace. Remove the 1 000 ms override and let the factory default apply; revisit `@stynx-nyx/data`'s real mutation score once Timeout rate drops below ~2 %._ |
| `timeoutFactor`            | unset (Stryker default 1.5)                                                                         | Acceptable except in combination with `@stynx-nyx/data`'s `timeoutMS: 1000` — there the factor is moot.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `concurrency`              | `2` (factory); `6` (`@stynx-nyx/data`)                                                              | Workspace default `2` is conservative on modern CI runners (4–8 vCPU). _Opinion: bump default to `Math.max(2, os.cpus().length - 2)` for local; explicit `concurrency: 4` on CI runners._ `@stynx-nyx/data`'s `concurrency: 6` is fine.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `ignoreStatic`             | factory default `false`; `@stynx-nyx/flow` overrides to `true` (`packages/flow/stryker.conf.mjs:6`) | The `flow` override has a defensible reason (Nest DI bootstrap re-runs every test). 11 % – 18 % `Ignored` mutants in flow (309/1736) lines up with that. _Opinion: review the same pattern for `@stynx-nyx/backend` (similar Nest-bootstrap surface) — currently `ignoreStatic: false`, which can inflate timeout/no-coverage rates on heavy DI modules._                                                                                                                                                                                                                                                                                                                                                                         |
...
I used the on-disk artefacts (every package's `<pkg>/.test-results/mutation.json`) as the execution layer. Every file was dated within 24 hours of this audit and recorded a `pnpm exec stryker run stryker.conf.mjs` execution.

| Package                       | Score % | break | Total |        Killed | Survived | NoCov | Timeout | CompileError | Ignored | Pass?                                    |
| ----------------------------- | ------: | ----: | ----: | ------------: | -------: | ----: | ------: | -----------: | ------: | ---------------------------------------- |
| `@stynx-nyx/auth`             |   92.50 |    85 |   657 |           356 |       29 |     0 |       2 |          270 |       0 | ✓                                        |
| `@stynx-nyx/data`             |   96.58 |    85 |   771 |           472 |       20 |     0 |  **94** |          185 |       0 | ✓                                        |
| `@stynx-nyx/tenancy`          |   90.33 |    80 |   341 | (n/a in dump) |       20 |     ? |       ? |            ? |       ? | ✓                                        |
| `@stynx-nyx/contracts`        |   83.33 |    60 |     — |             — |        — |     — |       — |            — |       — | ✓ (small surface; 9 totals in canonical) |
| `@stynx-nyx/core`             |   72.52 |    60 |   295 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/flow`             |   60.55 |    60 | 1 736 |           522 |  **340** |     7 |       0 |          558 | **309** | ✓ (margin **+0.55**)                     |
| `@stynx-nyx/sessions`         |   60.13 |    60 |   374 |            94 |   **63** |     0 |       1 |          216 |       0 | ✓ (margin **+0.13**)                     |
| `@stynx-nyx/idempotency`      |   63.82 |    60 |   255 |            86 |       51 |     — |       4 |          114 |       — | ✓                                        |
| `@stynx-nyx/logging`          |   66.66 |    60 |   152 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/testing`          |   67.64 |    60 |   513 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/cli`              |   68.05 |    60 |   404 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/sdk`              |   68.32 |    60 |   563 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/backend`          |   69.87 |    60 | 1 792 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular`          |   71.90 |    60 |   173 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/ratelimit`        |   71.11 |    60 |   285 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-storage`  |   84.25 |    60 |   546 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-auth`     |   73.04 |    60 |   418 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-iam`      |   74.77 |    70 |   337 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-audit`    |   95.12 |    70 |    60 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-flow`     |   76.47 |    60 | 1 050 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-tenancy`  |   77.08 |    60 |   156 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-trash`    |   87.15 |    60 |   211 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-ui`       |   83.72 |    60 |    69 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-i18n`     |   89.65 |    60 |   116 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-profile`  |   70.11 |    60 |   178 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/angular-sessions` |   98.24 |    60 |   133 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/audit`            |   75.38 |    60 |   539 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/health`           |   74.13 |    60 |   159 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/i18n`             |   80.85 |    60 |   233 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/privacy`          |   75.42 |    60 |   409 |             — |        — |     — |       — |            — |       — | ✓                                        |
| `@stynx-nyx/storage`          |   71.80 |    60 |   309 |             — |        — |     — |       — |            — |       — | ✓                                        |
...
Wall-clock: the canonical artefact's `durationMs` field gives the per-package mutation cycle time (e.g. `@stynx-nyx/sessions` 10 238 ms — read from `.test-results/mutation.json#durationMs`). The 31 cycles together amount to a few minutes locally; the weekly cron is well-budgeted.

**Two passing-but-fragile cases worth surfacing:**

- `@stynx-nyx/sessions` 60.13 % vs break 60 — margin **+0.13**. One mutant added or one passing test removed flips the gate red.
- `@stynx-nyx/flow` 60.55 % vs break 60 — margin **+0.55**.

**One score-inflation case:** `@stynx-nyx/data` 96.58 % is computed with Timeout-counted-as-killed, but **94 of 791 valid mutants are Timeouts** caused by the configured `timeoutMS: 1000`. The "real" mutation score with Timeouts excluded from the killed count is `(472 - 94) / (472 + 20 + 0 - 0) - actually = Killed / (Killed + Survived + Timeout)` per Stryker convention, but **the 1 000 ms timeout is artificially producing timeouts that may or may not be true survivors**. _Opinion: the 96.58 % figure is not trustworthy without a re-run at the factory-default timeout; the true number could be materially lower._

## Survivor classification

### Sample: `@stynx-nyx/sessions` (lowest-margin package, 63 survivors total)
...
### Pattern detection — `@stynx-nyx/sessions`
...
|  10 | medium   | `@stynx-nyx/idempotency` 63.82 % and `@stynx-nyx/logging` 66.66 % sit < 7 points from break. `ArithmeticOperator` survivors (8 in idempotency) indicate TTL / boundary math is asserted only on "happens at all" basis.                                                     | `packages/idempotency/reports/mutation/mutation.json` mutator breakdown; `.test-results/mutation.json#metric.score`.                                                        | Same playbook as #1/#2: open the HTML report, prioritise `ArithmeticOperator` and `ConditionalExpression`, author boundary tests (`expect(ttl).toBe(60_000)` instead of `expect(ttl).toBeDefined()`).                                                                                                    | M                                                                                                  |
...
|  14 | low      | `packages-web/angular-audit/` is missing a current `reports/stryker-incremental.json` file (the only one of 31 missing it).                                                                                                                                                 | `find packages-web/angular-audit -name 'stryker-incremental.json'` → empty; canonical `mutation.json` exists.                                                               | Re-run `pnpm --filter @stynx-nyx/angular-audit stryker` locally to seed the incremental file. Not a CI concern (CI runs full).                                                                                                                                                                           | XS                                                                                                 |
|  15 | low      | `@stynx-nyx/backend`'s `*.module.ts` files are correctly excluded from `mutate:` but `ignoreStatic: false` (factory default) — Nest-bootstrap re-runs on every test may inflate Ignored counts.                                                                             | `packages/backend/stryker.conf.mjs` has no `ignoreStatic` override; `tools/stryker/base.mjs:30` defaults to `false`. `@stynx-nyx/flow` already overrides to `true`.         | _Opinion (judgment call):_ consider the same `ignoreStatic: true` override on `@stynx-nyx/backend` and any package with > 10 `*.module.ts`-style files. Validate by inspecting the Ignored mutant ratio in the report; if > 10 %, the override pays off.                                                 | S                                                                                                  |
...
3. **`packages-web/angular-audit` and `packages-web/angular-iam` were added with `mutation: 70` overrides.** Should other Tier-1 web packages (`sdk`, `angular-auth`, `angular-flow`) get the same 70 floor, or remain at 60? This is a policy call that belongs in `tools/repo-config/test-policy.json#perPackage`.
...
- **What was sampled.** All 31 packages were inventoried at the config and per-(pkg, level) artefact level. The survivor classification deep-dive sampled `@stynx-nyx/sessions` (15 of 63 survivors); `@stynx-nyx/flow`, `@stynx-nyx/idempotency` were profiled at the mutator-breakdown level only. The 15 classifications are the basis for the cross-cutting patterns; a fuller dive into `flow` and `idempotency` is likely to confirm the same patterns but may surface a few `C` (equivalent-mutant) cases not visible at the mutator-distribution level.
- **What was not.** I did **not** re-run Stryker. I used the most recent on-disk JSON artefacts; every file is < 24 h old (timestamps in §"Inventory"). A re-run with the recommended `@stynx-nyx/data` config diff (remove `timeoutMS: 1000`) is the single most impactful follow-up and would change the data row for that package.
- **What would change the conclusions.** (a) If the 76 bare `.toHaveBeenCalled()` calls turn out to be predominantly `.not.toHaveBeenCalled()` mis-counted by the grep (re-checked: they are not), the anti-pattern severity drops. (b) If `@stynx-nyx/data`'s 94 Timeouts at `timeoutMS: 1000` resolve to actual survivors once timeout is restored, that package moves from #3-in-the-workspace to "needs work". (c) If the on-disk artefacts were not produced by the canonical pipeline (e.g. a developer ran with a custom flag), the score numbers above are stale; the `coverage/test-evidence.json` `generatedAt` and the per-package `endedAt` confirm freshness.
