...
//
// Shells out to `devai agent skill list` (JSON output from the CLI) and
...
export function loadSkills({ repoRoot }) {
  // npm-package consumption (project.json devai_consumption) resolves the
  // installed @devai-nyx/cli; packages/cli/dist/bin.js is the
  // sibling-checkout fallback when --repo-root points at a devai source tree.
  const installedBin = join(repoRoot, 'node_modules/@devai-nyx/cli/dist/bin.js');
  const bin = existsSync(installedBin) ? installedBin : join(repoRoot, 'packages/cli/dist/bin.js');
  let stdout;
  try {
    stdout = execFileSync('node', [bin, 'agent', 'skill', 'list'], {
      cwd: repoRoot,
      encoding: 'utf8',
      timeout: 30_000,
    });
  ...
  try {
    ...
    return { ok: false, error: `parse: ${err.message}`, skills: [] };
  }
...
function fmtField(label, value) {
  ...
  if (Array.isArray(value))
    return value.length === 0 ? null : `**${label}:** ${value.map((v) => `\`${v}\``).join(', ')}`;
  return `**${label}:** \`${value}\``;
}
...
function renderSkillPage(skill, position) {
  ...
  lines.push(
    '- [Roles → agent disciplines](../../roles/agent-disciplines.md) — which discipline invokes which skill.',
  );
  lines.push('');
  return lines.join('\n');
}
...
function renderIndex(skills) {
  ...
  lines.push('');
  lines.push(
    `> Auto-generated from \`devai agent skill list\` by \`scripts/gen-skill-catalog.mjs\` per [ADR-DOCS-IA Decision 6](../../meta/adr/ADR-DOCS-IA.md). **${skills.length}** skills registered across **${families.length}** families.`,
  );
  lines.push('');
...
export function buildSkillCatalog({ repoRoot }) {
  ...
  if (!ok) {
    return {
      index: `# Skills\n\n> **Catalog unavailable.** \`devai agent skill list\` failed: ${error}.\n`,
      pages: [],
    };
  }
