...
for (const release of releases) {
  const lines = [
    ...
    '- Do not mark this draft as shipped until Prompts 31 and 34-36 are fully green.',
    '',
  ];
...
const indexLines = [
  ...
  '',
  `Generated from \`${statusPath.replace(`${repoRoot}/`, '')}\`.`,
  '',
  '## Packages',
  '',
  ...(releases.length > 0
    ? releases.map((release) => `- ${release.name} -> ${sanitize(release.name)}.md`)
    : ['- No pending releases reported by Changesets.']),
  '',
];

writeFileSync(resolve(outDir, 'README.md'), indexLines.join('\n'));
console.log(`Generated ${releases.length} release draft notes in ${outDir}.`);
