...
**Produces:** `porting-pack/18-GAPS-AND-OPEN-QUESTIONS.md`.
**Depends on:** all prior pack files (collects `[GAP]` markers).
**Branch:** `porting-pack/18-gaps`.
...
1. `grep -rn "\[GAP" porting-pack/` — collect every gap marker.
2. `grep -rn "NOT YET IMPLEMENTED" porting-pack/` — collect every
