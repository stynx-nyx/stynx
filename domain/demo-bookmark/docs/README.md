...
| API skeleton           | ⏳ stub      | Scaffolded controllers/services/DTOs under `api/src/demo-bookmark/`. References `BookmarkPolicyGuard` + `@Resource`/`@Action` decorators that the scaffolder did NOT create (the matching `guards/policy.guard.ts` + `decorators/policy.decorator.ts` files are missing). **Hand-finishing follow-up:** either author those locally or rewire to `@stynx/backend`'s existing `AuthorizationGuard`. |
| API tests              | ⏳ stub      | Spec files under `api/test/` are placeholders. `pnpm test` is currently a no-op echo.                                                                                                                                                                                                                                                                                                              |
| UI feature module      | ⏳ stub      | Angular components scaffolded under `web/src/app/demo-bookmark/`. Not wired to a real API service.                                                                                                                                                                                                                                                                                                 |
| Docs                   | ✅ this file | + `ADR-0001.md` (also scaffolded; may need hand-finishing).                                                                                                                                                                                                                                                                                                                                        |
