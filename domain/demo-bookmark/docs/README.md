...
| UI service wiring      | ✅ R17 W07   | `bookmark.service.ts` calls `/api/demo/bookmark/bookmark` through Angular `HttpClient`; tenant/auth/request headers are left to `@stynx-nyx/angular` providers/interceptors rather than hand-rolled in the feature service. |
...
1. **Guards + decorators reconciled** — kept the T2 canonical `@stynx-nyx/auth` controller guard path (`StynxAuthGuard`, `PermissionGuard`, `@Permission`) because `@stynx-nyx/backend` documents those guards as the mounted backend auth/authorization substrate rather than a route-local replacement.
2. **Services wired to `@stynx-nyx/data`** — replaced scaffold stubs with real tenant-scoped `Database` transactions, active-row reads, parent-scoped tag queries, `ON CONFLICT DO NOTHING` tag creation, and `deleted_at` soft delete for bookmarks.
