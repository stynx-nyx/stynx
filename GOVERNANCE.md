# GOVERNANCE.md

> **Phase G of the C-4 DEVAI adoption pilot (2026-05).** This file is now a pointer; substantive governance lives where DEVAI says it lives.

Stynx is governed by **DEVAI** ([sibling repo](../devai)). Per session directive 5.2 (DEVAI is authoritative over stynx; DEVAI's governance supersedes legacy stynx governance), the canonical governance surfaces are:

| Concern                                                         | Canonical location                                                                     | Notes                                                                                                          |
| --------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Constitutional discipline (5 roles, substrates, evidence chain) | [`../devai/CONSTITUTION.md`](../devai/CONSTITUTION.md)                                 | Article 6 path-authority is the source of truth for who-may-edit-what.                                         |
| Engineering specifications                                      | [`docs/framework/arch/`](docs/framework/arch/)                                         | Created by `devai init --execute` in Phase A.2.                                                                |
| Architectural decisions                                         | [`docs/meta/adr/`](docs/meta/adr/)                                                     | DEVAI-shaped ADR substrate. (Pre-pilot ADRs migrated here in C-4 Session S5; `/specs/` retired in Session T6.) |
| Active invariants                                               | [`docs/framework/arch/invariants/`](docs/framework/arch/invariants/)                   | Currently: `INV-RBAC-001`, `INV-PRIVACY-001` (Phase B).                                                        |
| Contracts                                                       | [`docs/framework/contracts/`](docs/framework/contracts/)                               | DEVAI substrate; populate as schemas/contracts are extracted.                                                  |
| Operations runbooks                                             | [`docs/meta/ops/`](docs/meta/ops/)                                                     | Both DEVAI substrate (README.md) and legacy stynx (`recovery/`, `runbooks/`).                                  |
| Security posture                                                | [`docs/meta/security/`](docs/meta/security/)                                           | DEVAI substrate.                                                                                               |
| Glossary                                                        | [`docs/framework/glossary/`](docs/framework/glossary/)                                 | DEVAI substrate.                                                                                               |
| Inventory & evidence                                            | [`.devai/state/`](.devai/state/)                                                       | Auto-generated (do not hand-edit). Sensors, candidates, telemetry, evidence chain.                             |
| Module blueprints                                               | [`docs/framework/product/draft/blueprints/`](docs/framework/product/draft/blueprints/) | Currently: `BP-DEMO-BOOKMARK-001` (Phase C).                                                                   |
| CI gates                                                        | [`.github/workflows/devai-gates.yml`](.github/workflows/devai-gates.yml)               | Runs DEVAI's seven L0 sensors + invariant + blueprint validation on every PR (Phase E).                        |
| Commit format                                                   | [`tools/repo-config/commitlint.config.cjs`](tools/repo-config/commitlint.config.cjs)   | Accepts both DEVAI role-prefix subjects (`Architect: ...`) and Conventional Commits (Phase G).                 |

## What this file used to point at

The pre-pilot `docs/meta/gov/{health, audit, compliance}` tree has been archived to [`docs/meta/legacy/governance-archive/`](docs/meta/legacy/governance-archive/) (C-4 Session S5-3). Its content is kept for archeological reference but is NOT authoritative for current stynx governance — run the DEVAI commands above for live signal.

## How to participate

- **Contributors**: see [CONTRIBUTING.md](CONTRIBUTING.md) for commit and PR conventions.
- **Adopters considering DEVAI**: see [`../devai/docs/adopters/`](../devai/docs/adopters/) — stynx is the C-4 reference adopter; the [Phase A retro](docs/adopters/pilots/c-4/phase-a-retro.md) and [Phase I retro](docs/adopters/pilots/c-4/phase-i-retro.md) document our experience (full set under [`docs/adopters/pilots/c-4/`](docs/adopters/pilots/c-4/)).
- **Anyone touching governance**: cite the relevant Constitution article in your commit message body. Per the C-4 pilot, DEVAI's framing wins in any conflict with stynx-legacy framing.
